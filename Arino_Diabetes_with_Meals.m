function X = Arino_Diabetes_with_Meals()
clc, clear all, close all,

% time span domain
minmax = 4320; % number of mintues
tspan   = 0:.1:minmax;
...
function dx=dGdI(t,x,p)
    ...
    Obs_hr = 72; % hour observation time
    Obs = Obs_hr*60; % minute observation time
    d=w:T:Obs;
    D=mealGlucose/w*pulstran(t,d,'rectpuls',w);
    ...
    T1(count) = t;
    X1(count) = x(1);
    dx(1)=-p(1)*x(1)-p(4).*x(2).*x(1)+p(7) + D; % G(t)
