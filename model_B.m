...
E_A_50_S=1024;         % metformin amount at the biophase
                       % that produces 50% of maximal effect [micro-gram]
n_S=5;                % shape factor
...
INC=10;     % Amplitude of increase in the amount of Glucose due to eating
             % [ref: Postparandial ...](We shuld take an average of it. It 
             % is like Riemann theory!!) (It should be mg/dL)
w=500;       % sec-width of every pulse
             % [ref: Postparandial ...]
T_hr=2;      % hr-period
T=T_hr*3600; % sec-Period
d=w:T:f_ex;  % (determines where the middle of the pulse locates):Period:
             % (until what range-duration of the observations in seconds)
%Feed=INC*pulstran(tspan,d,'rectpuls',w);
